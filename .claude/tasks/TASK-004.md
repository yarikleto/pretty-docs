...
**Acceptance Criteria:**
- [ ] A minimal `SourceRef.astro` component renders an inline source citation with left border, section reference text, and optional quote
- [ ] A minimal SVG diagram Astro component (can be a simplified version of ThreeDimensions or BlockHeader) renders inline SVG in the article
- [ ] The diagram adapts to dark mode via CSS classes (`.diagram-bg`, `.diagram-card`, `.diagram-text-primary`)
- [ ] Both components are imported and used in `ftp-data-transfer.mdx`

**Visual Criteria:**
- [ ] Source citation has `border-left: 2px solid surface-300`, `font-size: 0.75rem`, `color: surface-600`
- [ ] Diagram sits inside a `bg-surface-50 rounded-2xl` container with padding
