...
**Acceptance Criteria:**
- [x] `src/layouts/Landing.astro` exists with HTML shell, font loading (same Google Fonts as Starlight config), meta tags, and inline dark mode detection script that reads from `localStorage` key `starlight-theme` and falls back to `prefers-color-scheme`
- [x] `src/pages/index.astro` uses the Landing layout and renders a placeholder hero with the headline "Old docs are ugly. These aren't." and a "Browse articles" link that navigates to `/networking/ftp-data-transfer/`
- [x] Dark mode state persists between the landing page and Starlight article pages (toggling on one is reflected on the other)
- [x] `src/content/docs/index.mdx` is removed (landing page is now at `src/pages/index.astro`)
