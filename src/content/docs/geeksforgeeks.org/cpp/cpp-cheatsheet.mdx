...
slug: geeksforgeeks.org/cpp/cpp-cheatsheet
prev: false
next: false
---
...
A **reference** is an alias for an existing variable. It shares the same memory address as the original and cannot be reassigned to refer to a different variable after initialization.
...
A **pointer** is a variable that stores the memory address of another variable. The address-of operator `&` retrieves a variable's address. The dereference operator `*` accesses the value at a pointer's address.
...
| `std::logic_error` | `<stdexcept>` | Errors due to faulty logic |
...
**Smart pointers** (C++11, `<memory>`) automate memory management by destroying the owned object when the pointer goes out of scope.

### `unique_ptr`

**`unique_ptr`** owns an object exclusively — it cannot be copied, only moved.
...
unique_ptr<int> p2 = move(p);       // OK: transfers ownership
```

### `shared_ptr`

**`shared_ptr`** allows multiple pointers to share ownership. The object is destroyed when the last `shared_ptr` is destroyed.
...
**`weak_ptr`** observes an object owned by `shared_ptr` without affecting the reference count. Used to break circular references.
...
The **Standard Template Library** provides a set of generic container classes.

### Associative Containers

| Container | Description | Header | Ordered |
|-----------|-------------|--------|---------|
| `map<K,V>` | Key-value pairs, unique keys | `<map>` | Yes |
| `multimap<K,V>` | Key-value pairs, duplicate keys allowed | `<map>` | Yes |
| `set<T>` | Unique elements | `<set>` | Yes |
| `multiset<T>` | Duplicate elements allowed | `<set>` | Yes |
| `unordered_map<K,V>` | Hash map, unique keys | `<unordered_map>` | No |
| `unordered_set<T>` | Hash set, unique elements | `<unordered_set>` | No |
...
set<int> s = {3, 1, 4, 1, 5};  // {1, 3, 4, 5} — duplicates removed, sorted
```

### Container Adaptors

| Container | Description | Header | Underlying |
|-----------|-------------|--------|------------|
| `stack<T>` | LIFO structure | `<stack>` | `deque` |
| `queue<T>` | FIFO structure | `<queue>` | `deque` |
| `priority_queue<T>` | Max-heap by default | `<queue>` | `vector` |
...
| Container | Description | Header | Random Access |
|-----------|-------------|--------|---------------|
| `vector<T>` | Dynamic array | `<vector>` | Yes |
| `deque<T>` | Double-ended queue | `<deque>` | Yes |
| `list<T>` | Doubly linked list | `<list>` | No |
| `forward_list<T>` | Singly linked list | `<forward_list>` | No |
| `array<T,N>` | Fixed-size array | `<array>` | Yes |
